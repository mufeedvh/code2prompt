...
class Gnaw:
    def __init__(self, path, include_patterns=None, exclude_patterns=None, 
        ...
        """
        Initialize a Gnaw configuration for generating prompts from code.
    ...
    def session(self) -> rust_sdk.PyGnawSession:
        """
        Create a PyGnawSession with the current configuration.
        """
        # Créer la session Rust avec la configuration actuelle
        session = rust_sdk.PyGnawSession(str(self.path))
