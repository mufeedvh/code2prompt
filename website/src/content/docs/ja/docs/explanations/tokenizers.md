...
表示されるトークン数は**推定値**です。Code2Promptは処理と任意の行番号追加を
行った後、各ファイルのトークンを並列に数え、テンプレートの推定コストを加算します。
完成したプロンプト全体を再度トークン化することはありません。断片の境界や
テンプレートによる変換で実際の数は変わるため、推定値が上限になる保証はありません。
数値フィールド名`token_count`は変わらず、JSONの外側の構造とシリアライズ時の
エスケープは計算に含まれません。
