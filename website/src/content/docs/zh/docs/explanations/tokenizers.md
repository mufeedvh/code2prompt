...
报告的 token 数是**估算值**：Code2Prompt 在处理文件并按需添加行号后，
并行统计各文件的 token 数，再加上模板的估算开销。最终提示词不会被整体重新分词。
片段边界和模板转换可能使实际数量不同，因此估算值并不是保证的上限。
数值字段 `token_count` 保留原名，不包含 JSON 外层结构和序列化时的转义开销。
