---
title: Apprendre les modèles Handlebars avec Code2Prompt
...
<Card title="Vue d'ensemble du tutoriel">
  Ce tutoriel démontre comment utiliser et créer des modèles Handlebars personnalisés
  pour la génération d'invites dans l'outil de ligne de commande `code2prompt`.
</Card>
...
## Qu'est-ce qu'un modèle Handlebars ?

[Handlebars](https://handlebarsjs.com/) est un moteur de templating populaire qui permet de créer des modèles dynamiques à l'aide de placeurs.
...
Les modèles Handlebars utilisent une syntaxe simple pour les placeurs et les expressions. Vous placerez les variables entre des doubles accolades `{{variable_name}}` pour les inclure dans l'invite générée.
...
- `source_tree` : L'arbre de sources du codebase, qui comprend tous les fichiers et répertoires.
...
Vous pouvez également utiliser des aides Handlebars pour effectuer des logiques conditionnelles, des boucles et d'autres opérations dans vos modèles. Par exemple :
...
Utilisez ce modèle pour générer des invites pour nettoyer et améliorer la qualité du code. Il recherchera des opportunités pour améliorer la lisibilité, l'adhérence aux bonnes pratiques, l'efficacité, la gestion des erreurs, etc.
...
Utilisez ce modèle pour créer une description de demande de tirage GitHub en Markdown en comparant le diff Git et le journal Git de deux branches.
...
Lors de la génération d'invites, `code2prompt` demandera à l'utilisateur de saisir des valeurs pour ces variables définies par l'utilisateur. Cela permet une personnalisation supplémentaire des invites générées en fonction des entrées de l'utilisateur.
