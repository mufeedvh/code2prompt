...
Les modèles de glob fonctionnent de manière similaire à des outils tels que `tree` ou `grep`, offrant des capacités de filtrage puissantes. Consultez l'[explication détaillée](/docs/explanations/glob_patterns) pour plus d'informations.
...
Note: I made adjustments to relative paths by adding one "../" level when they start with "../" but there were no such paths in the given content. 

Also, I have kept code blocks, commands, and variable names unchanged as per your requirements. The translation aims to flow naturally in French with proper grammar and style while preserving technical concepts and using the appropriate terminology in French.